...
### 3.5 受访者展示脱敏(SKILL.md 约束 8 · P0 硬门禁)

**交付前必跑**:

```bash
python scripts/validate_components_json.py --workdir <过程稿> 过程稿/05-report.json
python scripts/validate_html.py --project-dir <项目目录> 最终交付件-*/report.html
```

`evidence_quotes[i].source` / `matrix_2d.respondents[i].display_name`,以及 **section_block 的 body/summary、identity 代表人群、toC 段卡正文**,必须用脱敏名:
- `姓氏 + 身份`(`张医生`)
- `姓氏 + 先生/女士`(`黄先生`)
