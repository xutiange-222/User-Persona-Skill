...
2. 不替用户做选择，只问得更准。
3. 渲染层不静默截断内容。
4. 对用户屏蔽实现细节（网格、JSON 等）。
5. 证据全量，不「举例代表」。
6. 维度边界清晰（分类依据 vs 画像字段）。
7. 确定性渲染 + Schema 校验。
8. 过程稿可恢复。
9. 交付前跑质量检查。
...
## Core Principles (Nine Hard Constraints)

1. Anchor on research goals before judging importance.
2. Do not choose for the user—ask sharper questions.
3. No silent UI truncation.
4. User-facing language only in decisions.
5. Full evidence, not representative samples only.
6. Clear dimension boundaries.
7. Deterministic render + schema gates.
8. Recoverable process JSON.
