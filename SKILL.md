...
3. 00 至 03 先写中文 MD 给用户确认，确认后固化 JSON。01 必须先用中文解释五种画像方式，再给出推荐；`R1` 至 `R5` 只写入 JSON 和内部说明，禁止出现在用户对话或 01 MD。04 先写结构化草稿，再用 `workflow.py prepare-04` 生成中文 MD。05 先写 `05-report.draft.json`，再用 `workflow.py prepare-05` 生成只展示页面编排增量的中文 MD，用户确认后封存为 `05-report.json`。复杂 2B/2D 旅程按脚本提示运行四次 `journey-review`：每轮 MD 只展示当前层，每次确认立即把用户原话和本轮内容指纹写回草稿并刷新 MD；第四轮完成后直接封存，不再请求第五次总确认。简单 2C 和小型旅程一次确认。用户只需阅读中文语义，机器 ID、组件类型和枚举留在 JSON。
...
| 01 画像方式 | 五种中文画像方式、推荐结果、判断依据 | `status: confirmed`；内部保存 R1 到 R5 | 下方画像方式路由 |
...
先向用户解释五种画像方式，再根据研究输入推荐一种。对用户只使用下表中的中文名称；机器范式仅用于 JSON、校验器和内部文件，不得复制到用户对话或 `01-paradigm.md`。

| 用户看到的画像方式 | 适合情况 | 机器范式 |
|---|---|---|
| A 合并为一个画像 | 全部资料描述同一类用户，需要一个综合画像 | R2 |
| B 沿用已有分组 | 资料已经按角色、人群或既有分组整理 | R1 |
| C 按一个关键差异分类 | 一个明确差异足以区分多类用户 | R3 |
| D 用两个区分点形成矩阵 | 两个差异需要交叉观察，适合二维对照 | R4 |
| E 用多个区分点形成分布 | 三到五个差异共同作用，需要多维比较 | R5 |

向用户推荐时先完整展示五种方式，再用两到三句话说明：推荐哪一种、当前资料为什么适合、选择后会得到什么。禁止直接说“推荐 R2 单角色”或只报内部代码。除 toC 明确等待用户选择外，用户可以确认推荐方案，也可以改选其它中文方式。
...
`choice_label` 与 `paradigm` 必须按上表一一对应，校验器会阻止错配。
